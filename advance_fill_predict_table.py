...
def predict(num_cluster, in_table='train_1', out_table='predict', drop=['none'], modelo='ocu_mean',
    ...
    # cargar modelo
    model = load_model(f'models/adv_model_{num_cluster}_{modelo}.h5')
    scaler = joblib.load(f'models/adv_scaler_{num_cluster}_{modelo}.job')
    ...
    df['ocu_pred'] = df['ocu_pred'].shift(6)
...
def main():
    ...
    target_var = 'ocu_mean'
    out_table = 'predict_adv'

    for cl in range(initial, final):
        print(f'Prediciendo cluster {cl} - Train data')
        predict(cl, in_table='1_train_data', out_table=out_table, modelo=modelo, drop=drop, target_var=target_var)
        print(f'Prediciendo cluster {cl} - Test data')
        predict(cl, in_table='1_test_data', out_table=out_table, modelo=modelo, drop=drop,

...
def entrenar_cluster(num_cluster, num_celdas_LSTM=50, epochs=200, patience=10, keep=['all'], drop=['none'],
    ...
    sql = f"SELECT * FROM 1_train_data where cluster={num_cluster};"
    ...
    # La variable objetivo es el trafico en 30 minutos, como el training dataset es cada 5, hay que desplazar las
    # lecturas 6 posiciones para arriba
    df1['var_obj'] = df1[var_obj].shift(-6)
    ...
    try:
        resultado_df = pd.read_csv('resultado-advance.csv')
        ...
        print('Resultado-advance.csv no encontrado')
        resultado_df = resultado_df.append(row, ignore_index=True)

    resultado_df.to_csv('resultado-advance.csv')

    if save:
        model.save(f'models/adv_model_{num_cluster}_{var_obj}_{label}.h5')
        # grabar scaler
        joblib.dump(scaler, f'models/adv_scaler_{num_cluster}_{var_obj}_{label}.job')

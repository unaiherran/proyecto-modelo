...
def calcular_de_fecha(fecha):
    ...
    lst_fecha =[fecha] * 200
    lst_hora = [hora] * 200
    lst_minutos = [minutos] * 200

    df3 = pd.DataFrame(list(zip(cluster, lst_dia_semana, lst_dia_mes, lst_festivo, lst_fecha, lst_hora, lst_minutos)),
                       columns=['cluster', 'dia_semana', 'dia_mes', 'festivo', 'fecha', 'hora', 'minutos'])
...
def main():
    ...
    bucle(fecha_ini, fecha_fin, tb, inc_minutes=5)
